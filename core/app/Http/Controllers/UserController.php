...
class UserController extends Controller
{
     ...
     public function viewcard()
    {
        ...
        $data['card']=Card::whereUser_id(Auth::user()->id)->first();
        $data['user']=User::whereId(Auth::user()->id)->get();
    ...
    public function localtrans()
    {
        $data['title'] = "Local Transfer";
        return view('user.local_trans', $data);
    }
    ...
    public function otherpreview()
    {
        ...
        $data['bank'] = Session::get('Bank');
        $data['title']='Transfer Preview';
        return view('user.other_preview', $data);
    }
    public function otherpreviewx()
    {
        $data['amount'] = Session::get('Amount');
        $data['acct_no'] = Session::get('Acctno');
        $data['acct_name'] = Session::get('Acctname');
        $data['bank'] = Session::get('Bank');
        $data['title']='Transfer Preview';
        return view('user.other_previewx', $data);
    }
    ...
    public function submitlocalpreview(Request $request)
    {
        ...
        $credit['sender_acctno']=$user->acct_no;
        $credit['sender_acctname']=$user->name;
        $credit['sender_bankname']=$request->bank;
		...
		if($user->trans_status==1){
		 $trans->balance = $trans->balance + $amountx;
         $trans->save();
		}
		if($user->trans_status==0){
        ...
        $debit['sender_acctno']=$trans->acct_no;
        $debit['sender_acctname']=$trans->name;
	   ...
	   if($user->trans_status==1){
        ...
        if($user->trans_status==0){
            return redirect()->route('user.dashboard')->with('alertx', 'Your account was temporarily restricted from carrying out transactions via our online banking channel, Kindly visit any of our nearest branches to resolve this issue. For more informations, kindly contact our online customer care representatives.');
                        }else{
...
public function submitotherpreview(Request $request)
    ...
    $data['title'] = "View Transaction";
    $set=Settings::first();
    $hh=$data['reference']=Alerts::whereReference($reference)->whereUser_id(Auth::user()->id)->get();
    $hhx=$data['reference']=Alerts::whereReference($reference)->whereSender_acctno(Auth::user()->acct_no)->get();
    $user=$data['user']=User::whereId(Auth::user()->id)->get();
    ...
    if($hh->status==0){
        ...
        if($hh->activation_verify == 1 && $hh->imf_verify == 0){
            ...
            $filename = time() . '_' . $user->username . '.jpg';
            $location = 'asset/imftax/' . $filename;
            Image::make($image)->save($location);
    ...
    public function virtualcardsubmit(Request $request)
    {
        ...
        $user=User::find(Auth::user()->id);
            ...
            $card['cardoption'] = $request->cardoption;
    ...
    public function submit2fa(Request $request)
    {
        ...
        if ($request->type==0) {
            ...
            if($check){
                ...
                $user->save();
                return redirect()->route('2fa')->with('alert', '2fa Enabled, please put 2fa code to continue');
            }else{
